...
.factory('Hub', ['$', function ($) {
	...
	return function (hubName, options) {
		...
		Hub.connect = function (queryParams) {
			...
			//if (angular.isDefined(options.bearerToken)) {			    
			//    Hub.setTokenCookie(options.bearerToken);
			//};
			return Hub.connection.start(startOptions);		   
		};

		//Hub.setTokenCookie = function (token) {
		//    if (token)
		//        document.cookie = "BearerToken=" + token + "; path=/";
		//}

		//Hub.clearTokenCookie = function () {
